...
const streamIpcSource = fs.readFileSync(path.join(rootDir, 'backend', 'ipc', 'stream.js'), 'utf8');
...
test('stream URL proxies are stopped from before-quit as well as will-quit', () => {
    assert.match(streamIpcSource, /const cleanupActiveStreams = \(\) => \{/);
    assert.match(streamIpcSource, /app\.on\('before-quit', cleanupActiveStreams\)/);
    assert.match(streamIpcSource, /app\.on\('will-quit', cleanupActiveStreams\)/);
});
