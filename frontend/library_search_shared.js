...
function fuzzySearch(items = [], query = '', options = {}) {
    ...
    if (!cleanQuery) return Array.isArray(items) ? [...items] : [];
    const sourceItems = Array.isArray(items) ? items : [];
    const titleBuilder = typeof options.titleBuilder === 'function' ? options.titleBuilder : defaultTitleForItem;
    const queryTokens = tokenizeSearchText(cleanQuery);
    const fuse = createFuseEngine(sourceItems, { ...options, includeScore: true });
    ...
    return fuse.search(cleanQuery)
        .map((result, index) => {
            const item = buildSearchItem(result.item, titleBuilder);
            const match = classifySearchMatch(item, cleanQuery, queryTokens);
